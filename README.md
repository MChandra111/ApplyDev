...
> Tailored applications, grounded in evidence — not invented metrics.
...
1. **Scrape or accept a job description** — paste a URL and the scraper (`httpx` + BeautifulSoup) strips it to plain text, or paste JD text directly to skip scraping for sites that block bots.
2. **Research and parse run in parallel** — a `ResearchAgent` runs an observe → act loop against Tavily search to find company size, recent news, tech stack mentions, and red flags; a `JDParserAgent` extracts required skills and years-of-experience requirements and matches each against your resume via Pinecone RAG.
...
4. **An `EvalAgent` scores the opportunity 1–10** with reasoning, based on research and requirement fit, before you decide whether it's worth applying.
...
| AI / LLM | Groq (`llama-3.3-70b-versatile`) across all 5 agents — Research, JD Parser, Resume Profile, Writer, Eval |
...
Open `http://localhost:5173` — the header shows **Backend connected** once the API is reachable. Paste a job URL (or enable "Send sample JD text" in Dev options to skip scraping) and click **Analyze job**.
...
- Job results live in an in-memory Python dict — lost on backend restart; no database yet.
- Job history and the Application Tracker are `localStorage`-only: no accounts, no sync across devices or browsers.
...
- The eval harness's LLM-as-judge and the agents it grades both run on the same Groq model family — cross-model judging would catch more bias (noted as a caveat in `EVALS.md`).
- Per the project's phase roadmap: eval CI integration, cloud deployment, and general polish are still open (Phases 7–8).
